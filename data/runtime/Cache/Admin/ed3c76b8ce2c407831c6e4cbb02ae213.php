...
<body>
	<div class="wrap">
		<ul class="nav nav-tabs">
			...
			<li ><a href="<?php echo U('Push/add');?>">发送</a></li>
		...
		<form class="well form-search" name="form1" method="post" style="float:left" action="">
			...
			关键字： 
			<input type="text" name="keyword" style="width: 200px;" value="<?php echo ($formget["keyword"]); ?>" placeholder="请输入会员id、管理员id...">
			<input type="button" class="btn btn-primary" value="搜索" onclick="form1.action='<?php echo U('Push/index');?>';form1.submit();"/>
			<input type="button" class="btn btn-primary" style="background-color: #1dccaa;" value="导出" onclick="form1.action='<?php echo U('Push/export');?>';form1.submit();"/>	
		</form>	
			...
			<table class="table table-hover table-bordered">
				...
				<tbody>
					<?php if(is_array($lists)): foreach($lists as $key=>$vo): ?><tr>
						...
						<td><?php if($vo['touid'] == ''): ?>全部会员<?php else: echo ($vo['touid']); endif; ?></td>
