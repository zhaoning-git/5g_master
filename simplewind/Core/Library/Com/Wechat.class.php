...
class Wechat {
    ...
    //卡券/门店相关地址
    const STORE_LIST = '/poi/getpoilist?'; //拉取门店列表
    const STORE_INFO = '/poi/getpoi?'; //拉取门店列表
    const STORE_EDIT = '/poi/updatepoi?'; //拉取门店列表
    const STORE_ADD = '/poi/addpoi?'; //创建门店
    const STORE_DEL = '/poi/delpoi?'; //删除门店
    const STORE_IMG = '/media/uploadimg?'; //门店图片上传接口
    //卡券接口 
    const CARD_TEST = '/card/testwhitelist/set?'; //查看卡券详情
    const CARD_LIST = '/card/user/getcardlist?'; //获取用户已领取卡券
    const CARD_GET = '/card/get?'; //查看卡券详情
    const CARD_CODE_ENCRYPT = '/card/code/decrypt?'; //卡券解码
    const CARD_CODE_GET = '/card/code/get?'; //查询卡券CODE
    const CARD_CODE_CONSUME = '/card/code/consume?'; //核销卡券CODE
    const CARD_CODE_MARK = '/card/code/mark?'; //锁定CODE
    const CARD_JS_TICKET = '/ticket/getticket?'; //获取卡券凭证票据
    //微信WIFI
    const WIFI_LIST = '/bizwifi/shop/list?'; //wifi门店列表
    const WIFI_INFO = '/bizwifi/shop/get?'; //wifi门店详情
    const WIFI_EDIT = '/bizwifi/shop/update?'; //修改门店WIFI基本信息
    const WIFI_ADD_PW = '/bizwifi/device/add?'; //为门店设置密码类型的WIFI
    const WIFI_TPL = '/bizwifi/homepage/set?'; //设置商家主页
    const WIFI_TOP_BAR = '/bizwifi/bar/set?'; //顶部常驻入口文案
    const WIFI_OK_JUMP = '/bizwifi/finishpage/set?'; //连网完成跳转url
    const WIFI_DELETE = '/bizwifi/shop/clean?'; //删除门店的所有设备
    const WIFI_QRCODE = '/bizwifi/qrcode/get?'; //获取WIFI二维码
    const WIFI_LINK_URL = '/bizwifi/account/get_connecturl?'; //获取公众号联网URL
    ...
    public $access_token_refresh_url;
    ...
    public function __construct($options) {
        ...
        $this->mch_key = isset($options['mch_key']) ? $options['mch_key'] : '';
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    //
    public function access_token_refresh($authname, $field){
        $access_token_refresh_url = M('WechatConfig')->where(array('uid' => 1))->getField('access_token_refresh_url');
        if(empty($access_token_refresh_url)){
            //$result = $this->http_get(self::API_URL_PREFIX . self::AUTH_URL . 'appid=' . $appid . '&secret=' . $appsecret);
            $this->errCode = '00001';
            $this->errMsg = 'access_token公共刷新URL未设置';
            return false;
        }else{
            $result = $this->http_get($access_token_refresh_url);
            if($result){
                $json = json_decode($result, true);
                
                $expire = $json['Open']['expires_in'] ? intval($json['Open']['expires_in']) - 100 : 3600;
                
                
                if($field == 'access_token'){
                    $res = $json['Open']['access_token'];
                }
                
                elseif($field == 'jsapi_ticket'){
                    $res = $json['Open']['jsapi_ticket'];
                }
                
                $this->setCache($authname, $res, $expire);
                return $res;
            }
        }
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '') {
        ...
        $this->access_token = $this->access_token_refresh($authname, 'access_token');
        return $this->access_token;
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '') {
        ...
        $this->jsapi_ticket = $this->access_token_refresh($authname, 'jsapi_ticket');
        return $this->jsapi_ticket;
...
// :TODO: Is there any more datatype we should be in the lookout for? (Object?)
     ...
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
    public function getOauthAccessTokenall() {
        $code = isset($_GET['code']) ? $_GET['code'] : '';
        if (!$code)
            return false;
        $data = json_decode ( $this->get_php_file("/Application/Api/Controller/access_token.php"));
        if ($data->expire_time < time ()) {
            ...
            if ($result) {
                ...
                $this->user_token = $json['access_token'];
                 ...
                 $this->set_php_file ("/Application/Api/Controller/access_token.php", json_encode($data));
    ...
    public function getTMIndustry() {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/template/get_industry?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function gettemplate() {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . '/template/get_all_private_template?access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     */
    public function Store($begin = 0, $limit = 50) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::STORE_LIST . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     * @return boolean|array   返回数组请参看 微信卡券接口文档 的json格式
     */
    public function StoreInfo($poi_id) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::STORE_INFO . 'access_token=' . $this->access_token, self::json_encode($data));
    ...
    public function StoreEdit($data) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::STORE_EDIT . 'access_token=' . $this->access_token, self::json_encode($data));
    ...
    public function StoreAdd($data) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::STORE_ADD . 'access_token=' . $this->access_token, self::json_encode($data));
    ...
    public function StoreImg($data) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::STORE_IMG . 'access_token=' . $this->access_token, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if ($json['errcode'] != 0) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array   返回数组请参看 微信卡券接口文档 的json格式
     */
    public function StoreDel($poi_id) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::STORE_DEL . 'access_token=' . $this->access_token, self::json_encode($data));
    ...
    public function CardTest() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_TEST . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     * 获取用户卡券
     */
    public function CardList($openid, $card_id) {
        if (!$openid) {
            return false;
        }
        $data['openid'] = $openid;
     ...
     * 查询卡券CODE状态
     */
    public function CardCodeGet($code, $check_consume = true, $card_id) {
        if (!$code) {
            return false;
        }
        $data['code'] = $code;
     ...
     * 卡券CODE解码
     */
    public function CardCodeEncrypt($encrypt_code) {
        if (!$encrypt_code) {
            return false;
        }
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_CODE_ENCRYPT . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     * 卡券JSDK TICKET
     */
    public function CardTicet($sign_type = 'cardExt', $location_id, $card_id = '', $card_type, $code = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;

        $result = $this->http_get(self::API_URL_PREFIX . self::CARD_JS_TICKET . 'access_token=' . $this->access_token . '&type=wx_card');
        if ($result) {
            $json = json_decode($result, true);
            if ($json['errcode'] != 0) {
                $this->errCode = $json['errcode'];
            ...
            if ($sign_type == 'cardSign') {
                $arrData = array($json['ticket'], $this->appid, '', $timestamp, $noncestr, '', '');
                foreach ($arrData as $k => $v) {
                    $sign[$v] = substr($v, 0, 1);
                }
                asort($sign);
                $sign = SHA1(implode('', array_keys($sign)));
            }

            if ($sign_type == 'cardExt') {
                $arrData = array($json['ticket'], $timestamp, $card_id, '', '', $noncestr);
                foreach ($arrData as $k => $v) {
                    $sign[$v] = substr($v, 0, 1);
                }
                asort($sign);
            ...
            if (!$sign)
                return false;
            $signPackage = array(
                "openid" => 'oehYqw8GXePXcUDrcEltayg1d0BU',
                "card_id" => $card_id,
                "ticket" => $json['ticket'],
                "timestamp" => $timestamp,
                "nonceStr" => $noncestr,
     ...
     * 核销卡券
     */
    public function CardConsume($code, $openid) {
     ...
     * 锁定CODE
     */
    public function CardCodeMark($card_id, $code, $openid) {
        $data = array(
            'card_id' => $card_id,
            'code' => $code,
            'openid' => $openid,
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_CODE_MARK . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * WIFI门店
     */
    public function WifiList($pageindex) {
        if (!$pageindex) {
            return false;
        }
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_LIST . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     * WIFI门店详情
     */
    public function WifiInfo($shop_id) {
        if (!$shop_id) {
            return false;
        }
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_INFO . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     * 修改门店WIFI
     */
    public function WifiEdit($shop_id, $ssid, $password, $bar_type, $homepage_url, $finishpage_url) {
        ...
        $data['template_id'] = 1;
        $data['struct'] = array('url' => $homepage_url);
        $data['bar_type'] = intval($bar_type);
        $data['finishpage_url'] = $finishpage_url;
        if (!$this->access_token && !$this->checkAuth())
            return false;


        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_EDIT . 'access_token=' . $this->access_token, self::json_encode($data));
        $result1 = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_ADD_PW . 'access_token=' . $this->access_token, self::json_encode($data));
        $result2 = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_TPL . 'access_token=' . $this->access_token, self::json_encode($data));
        $result3 = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_TOP_BAR . 'access_token=' . $this->access_token, self::json_encode($data));
        $result4 = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_OK_JUMP . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     * 删除门店所有WIFI设备
     */
    public function WifiDelete($shop_id) {
        if (!$shop_id) {
            return false;
        }
        $data['shop_id'] = intval($shop_id);
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_DELETE . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     *  门店WIFI二维码
     */
    public function WifiQrcode($shop_id) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::WIFI_QRCODE . 'access_token=' . $this->access_token, self::json_encode($data));
     ...
     *  WIFI公众号链接
     */
    public function WifiLink() {

        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::WIFI_LINK_URL . 'access_token=' . $this->access_token);
    ...
    public function getAddress($url) {
        ...
        if (!$_GET['code']) {
            redirect($this->getOauthRedirect($url, 'upshop', 'snsapi_base'));
        }
        ...
        $Parameters['noncestr'] = rand(100000, 999999);
