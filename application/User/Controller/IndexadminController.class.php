...
class IndexadminController extends AdminbaseController {
    ...
    function index() {
        ...
        $head_default_pic = getDefaultHead();
    ...
    function myClubs(){
        ...
        if ($id) {
            $prefix= C("DB_PREFIX");
            $map['user_id'] = $id;
            $count = M('AttentionClub')->where($map)->count();
            $page = $this->page($count, 20);

            $lists = M('AttentionClub a')
                    ->join("{$prefix}clubs c on c.id = a.club_id")
                    ->join("{$prefix}picture p on c.headimg = p.id")
                    ->where($map)
                    ->order("a.id DESC")
                    ->limit($page->firstRow . ',' . $page->listRows)
                    ->field('c.id,c.name,p.path, a.add_time')
                    ->select();

            $this->assign('userinfo', M('Users')->where(['id'=>$id])->field('id,user_login,user_nicename,avatar')->find());
            $this->assign('uid', $id);        
            $this->assign('lists', $lists);
            $this->assign('formget', $_GET);
            $this->assign('count', $count);
            $this->assign("page", $page->show("Admin"));
            $this->display(":myClubs");
        }else{
            $this->error('数据传入失败！');
        }
    }
    ...
    function addMyClubs(){
        if(IS_POST){
            $data = I();
            $uid = intval($data['uid']);
            M('AttentionClub')->where(['user_id'=>$uid])->delete();
            if(!empty($data['club_id'])){
                $Ins['add_time'] = time();
                foreach($data['club_id'] as $value){
                    $Ins['user_id'] = $uid;
                    $Ins['club_id'] = $value;
                    M('AttentionClub')->add($Ins);
                }
            }
            $this->success("成功！");
        }else{
            ...
            $clubs = M('Clubs c')
            ->join("{$prefix}picture p on c.headimg = p.id")
            ->field('p.path as url,c.name,c.id')
            ->where($map)
            ->select();

            $clubs = array_map(function($val) use($id) {
                $val['is_checked'] = M('AttentionClub')->where(['user_id'=>$id,'club_id'=>$val['id']])->count();
                return $val;
            },$clubs);

            // /print_r($clubs);exit;
            $this->assign('userinfo', M('Users')->where(['id'=>$id])->field('id,user_login')->find());
            $this->assign('lists', $clubs);
            $this->display(":addMyClubs");
        }
    }
    ...
    function add_post() {
        if (IS_POST) {
            ...
            if ($user->create()) {
                ...
                $avatar = $_POST['avatar'];
                if ($avatar == '') {
                    $user->avatar = $head_default_pic;
                    $user->avatar_thumb = $head_default_pic;
