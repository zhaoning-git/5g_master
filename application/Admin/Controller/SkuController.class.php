...
class SkuController extends AdminbaseController {
    ...
    function index(){
        $prefix = C('DB_PREFIX');
    	$page = $this->page(M("cat")->count(), 10);
    	$lists = M('duihuan d')
            ->join("{$prefix}cat c on d.cat_id = c.id")
            ->field('c.id,c.type,d.shopname')
            ->limit($page->firstRow . ',' . $page->listRows)
            ->select();


    	foreach ($lists as $k=>&$v){
            $sum = count(M('attribute')->where(array('cat_id'=>$v['id']))->select());
            $v['sum'] = $sum;
        }
    ...
    function attrList(){
        $prefix = C('DB_PREFIX');
        $info = M('attribute a')
            ->join("{$prefix}cat c on a.cat_id = c.id")
            ->field('a.id,a.attribute_name,c.type as catname')
            ->where(array('a.cat_id'=>$_GET['id']))
            ->select();

        foreach ($info as $k=>&$v){
            $lists[] = M('duihuan_attribute')->field('shuzhi,duihuan_id')->where(array('shuzhi_id'=>$v['id']))->select();
    ...
    function addAttr(){
        ...
        // //获取实物列表
         $shiInfo = M('duihuan')->where(array('type'=>'实物'))->select();


        $this->assign('shiInfo',$shiInfo);
        $this->display();
    }
    ...
    function goodsType()
    {
        ...
        $prefix = C('DB_PREFIX');
        $info = M('duihuan d')
            ->join("{$prefix}attribute c on d.cat_id = c.cat_id")
            ->where(array('d.id' => $goodsid))
            ->select();
        $this->ajaxReturn(array('status'=>1,'info'=>'success','data'=>$info));
    }

    //获取商品属性值列表
    function attrValList(){
        $id = $_POST['id'];
        $info = M('duihuan_attribute')
            ->field('id,shuzhi')
            ->where(array('shuzhi_id'=>$id))
            ->select();
        $this->ajaxReturn(array('status'=>1,'info'=>'success','data'=>$info));
    }
