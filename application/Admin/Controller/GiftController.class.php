...
class GiftController extends AdminbaseController {
    ...
    public function listorders() { 
        ...
        foreach ($ids as $key => $r) {
            $data['orderno'] = $r;
            M("duihuan")->where(array('id' => $key))->save($data);
        }
    ...
    public function listorderss(){
        ...
        if($type == 1){
          $models = '实物';
          $uls = U('jifenduihuan/shiindex');
        }
        if($type == 2){
          $models = '特权';
          $uls = U('jifenduihuan/teindex');
        }
        if($type == 3){
          $models = '限量';
          $uls = U('jifenduihuan/xianindex');
        }
        if($type == 4){
          $models = '道具';
          $uls = U('jifenduihuan/daoindex');
        }
         if($type == 5){
          $models = '虚拟物品';
          $uls = U('jifenduihuan/xuindex');
        }if($type == 6){
          $models = '礼物';
          $uls = U('jifenduihuan/liindex');
        }
        if($type == 7){
          $models = '实物';
          $uls = U('jifenduihuan/xinindex');
        }
        foreach ($ids as $key => $r) {
            $data['sort'] = $r;
            $d = M('duihuan')->where(array('id' => $key,'type'=>$models))->save($data);
        }
    ...
    public function tels(){
       $ids = $_POST['listorders'];
        ...
        foreach ($ids as $key => $r) {
            $data['sort'] = $r;
            $d = M("duihuan")->where(array('id' => $key))->save($data);
        }
        ...
        if ($status) {
            ...
            // setAdminLog($action);
            // $this->resetCache();
            $this->success("排序更新成功！",U('jifenduihuan/teindex'));
        } else {
            $this->error("排序更新失败！");
        }
