...
class AuthGroupModel extends Model {
    ...
    const MEMBER = 'member';
    const UCENTER_ADMIN = 'ucenter_admin';
    ...
    public function getGroups($where = array()) {
        ...
        $map = array('status' => 1, 'type' => self::TYPE_ADMIN, 'module' => 'admin','id'=>array('not in',$USERGROUP[0]));
     ...
     */
    public function addToGroup_($uid, $gid) {
        $uid = is_array($uid) ? implode(',', $uid) : trim($uid, ',');
        $gid = is_array($gid) ? $gid : explode(',', trim($gid, ','));

        $Access = M(self::AUTH_GROUP_ACCESS);
        //if( isset($_REQUEST['batch']) ){  //为单个用户批量添加用户组时,先删除旧数据
    ...
    public function addToGroup($uid, $gid) {
        ...
        $NOEDITGROUP = C('NOEDITGROUP');
        if(in_array($uid,$NOEDITGROUP[0])){
            $this->error = '该用户被禁止修改用户组!';
        }
        
        $Access = M(self::AUTH_GROUP_ACCESS);
        $ogid = $Access->where(array('uid' => $uid))->getField('group_id');
        if(!empty($ogid)){
            $ogid_name = M(self::AUTH_GROUP)->where(array('id'=>$ogid))->getField('title');
            if(in_array($ogid,$NOEDITGROUP[1])){
                $this->error = $ogid_name.'用户不能修改用户组!';
            }
        }
        
        $del = $Access->where(array('uid' => $uid))->delete();
        $Access->add(array('uid'=>$uid,'group_id'=>$gid));
        User($uid,true,true);
        return true;
    }
    ...
    public function getGroup($uid,$field=''){
        $dsad = self::getUserGroup($uid);
        $getUserGroup = $dsad[0];
        if(!$getUserGroup['group_id']){
            $getUserGroup['group_id'] = 1;
        }
        
        if(!empty($field) && is_string($field)){
            return $getUserGroup[$field];
        }else{
            return $getUserGroup;
        }
    }
    ...
    static public function memberInGroup($group_id) {
        ...
        $r_table2 = $prefix . self::UCENTER_ADMIN;
