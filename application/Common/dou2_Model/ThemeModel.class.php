...
class ThemeModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('cat', 'require', '话题分类不能为空',self::MUST_VALIDATE,'regex', self::MODEL_INSERT),
    );
    ...
    function Insert($data=array()){
        ...
        $ThemeCategory = M('ThemeCategory')->where(array('id'=>intval($data['cat'])))->find();
        
        if(empty($ThemeCategory)){
            $this->error = '话题分类不正确!';
            return false;
        }
        
        $data['cat'] = $ThemeCategory['id'];
